---
title: "Curve-fitting Known Data for Chemical Physical Properties"
...
To use simple and quick techniques to estimate physical properties of chemicals using known data.
...
This mathematical method for solving a non-linear equation requires the derivative of this equation.  Thus, one would take the above equation, "f(X)", and differentiate on each of the coefficients in "X" using the Symbolic math.  The following are the results.

First derivatives of vapor pressure function:
<div align="center">
$$\begin{equation}
  \frac{df(A,B,C)}{dA} = -1
  \end{equation}$$
...
$$\begin{equation}
  \frac{df(A,B,C)}{dB} = \frac{1}{T_{i}+C}
  \end{equation}$$
</div>

<div align="center">
$$\begin{equation}
  \frac{df(A,B,C)}{dC} = \frac{B}{(T_{i}+C)^2}
  \end{equation}$$
</div>
...
SE_NR <- (SE_NR/(M-1))^0.5
print(paste("Standard Error for the quick estimate is ",
            round(SE_estimate,3),
            ", whereas the Standard Error for the Newton_Rhaphson method is ",
            round(SE_NR,3)))
```
